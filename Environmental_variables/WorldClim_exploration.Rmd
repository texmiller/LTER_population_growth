...
## trying it first with the lowest level of resolution
## it takes a long time to download at the higher resolutions
climate_dat <- raster::getData("worldclim",var="bio",res=10)

## This would be the command for pulling the predicted future climate
## but it's not working

#r <- getData('CMIP5', var='bio', res=10, rcp=85, model='AC', year=70)
#r2 <- getData('CMIP5', var='prec', res=10, rcp=85, model='AC', year=70)
#climate_dat = r
...
plot(climate_dat$Temp*.1, main="Annual Mean Temperature", xlab="Longitude",ylab="Latitude",cex.axis=1.3, cex.lab=1.4, cex.main=1.5,col=rev(heat.colors(20)))
plot(points,add=T)
```
...
plot(climate_dat$Prec,main="Annual Precipitation", xlab="Longitude",ylab="Latitude",cex.axis=1.3, cex.lab=1.4, cex.main=1.5)
plot(points,add=T)
```
...
## (need to multiply temp by 0.1 to get it into the correct format for degrees C)
plot((climate_dat$Temp*.1), xlim = c(-160, -50), ylim = c(0, 100), main="Annual Mean Temperature", xlab="Longitude",ylab="Latitude",cex.axis=1.3, cex.lab=1.4, cex.main=1.5,col=rev(heat.colors(10)))
plot(points,add=T)

## visualize the LTER sites with the precipitation data (mm) for N. America
plot(climate_dat$Prec,xlim = c(-160, -50), ylim = c(0, 100), axes=TRUE,main="Annual Precipitation", xlab="Longitude",ylab="Latitude",cex.axis=1.3, cex.lab=1.4, cex.main=1.5)
plot(points,add=T)
```
